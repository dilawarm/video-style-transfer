...
def detect_motion_boundary(u_1, u_2, v_1, v_2):
    return (u_2 - u_1) ** 2 + (v_2 - v_1) ** 2 > (
        0.01 * np.sqrt(u_1 ** 2 + v_1 ** 2) ** 2
    ) + 0.002
...
def loss_function(image, idx, c, omega):
    ...
    style_loss = tf.add_n(
        [
            wasserstein_distance(
                style_outputs[name].reshape(-1), style_targets[name].reshape(-1)
            )
    ...
    if idx > 0:
        for j in J:
            if idx - j < 0:
                break
            temporal_loss += tf.reduce_mean(((image - omega[j]) * c[j]) ** 2)
